...
#ifndef MULT_MEASURE_TIME_Hpp
# define  MULT_MEASURE_TIME_Hpp
    ...
    public:
        ...
        bool isExpireSec(seconds_interval sec, bool repeate = true) noexcept
        {
            if (sec >= (std::numeric_limits<std::int32_t>::max() / 1000)) {
                MULTFatal("=====> Targe Seconds very big OVER 24days");
                sec = std::numeric_limits<std::int32_t>::max() / 1000;
                MULTFatal("=====> We adjust reduce 24days(2147483.648 = 2147483[s])");
            }
        ...
        bool isExpire(millisec_interval msec, bool repeate = true) noexcept
        {
            ...
            if (m_flag) {
                ...
                MULTError("====> The timer not started...");
            }
            return ret;
        }
...
} // end of namespace Mult


#endif //<-- macro  MULT_MEASURE_TIME_Hpp ends here.
